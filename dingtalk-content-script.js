  ...
  async function handleApiDownload(sendResponse) {
    try {
      ...
      if (!data.isSuccess || data.status !== 200) {
        throw new Error(`API returned error: status=${data.status}, isSuccess=${data.isSuccess}`);
      }

      if (!data.data || !data.data.ossUrlPreSignatureInfo || !data.data.ossUrlPreSignatureInfo.preSignUrls || data.data.ossUrlPreSignatureInfo.preSignUrls.length === 0) {
        throw new Error('Invalid API response structure - missing download URL');
      }
      ...
      let filename = `dingtalk_document_${dentryUuid}.xlsx`;
      ...
      // 返回结果给background script
      sendResponse({
        success: true,
        downloadUrl: downloadUrl,
        filename: filename,
        msg: "Download URL obtained successfully"
      });
